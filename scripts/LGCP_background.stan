data {
    ...
    array[N] int<lower=1> species; // Species index for each observation
    matrix[N, K] X;               // Environmental covariate matrix
    array[N] int<lower=0, upper=1> y; // Presence (1) or background (0)
    matrix[J, J] D_phylo;         // Phylogenetic distance matrix
    vector[N] log_offset;         // Log effort or area per observation (e.g., log(duration × observers × distance))
}
...
model {
    ...
    for (n in 1:N) {
        int s = species[n];
        real log_lambda = dot_product(X[n], B[s, ]) + f[n] + log_offset[n];
